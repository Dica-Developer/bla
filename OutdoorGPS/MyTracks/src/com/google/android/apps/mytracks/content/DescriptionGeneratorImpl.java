...
public class DescriptionGeneratorImpl implements DescriptionGenerator {
   ...
   * @param html true to use "<br>" for line break instead of "\n"
   */
  public String generateXMLTripStatisticsDescription(TripStatistics stats) {
    String lineBreak = TEXT_LINE_BREAK;
    StringBuilder builder = new StringBuilder();

    // Total distance
    writeXMLDistance(
        stats.getTotalDistance(), builder, R.string.description_total_distance_XML, lineBreak);

    // Total time
    writeXMLTime(stats.getTotalTime(), builder, R.string.description_total_time_XML, lineBreak);

    // Moving time
    writeXMLTime(stats.getMovingTime(), builder, R.string.description_moving_time_XML, lineBreak);

    // Average speed
    Pair<Double, Double> averageSpeed = writeXMLSpeed(
        stats.getAverageSpeed(), builder, R.string.description_average_speed_XML, lineBreak);

    // Average moving speed
    Pair<Double, Double> averageMovingSpeed = writeXMLSpeed(stats.getAverageMovingSpeed(), builder,
        R.string.description_average_moving_speed_XML, lineBreak);

    // Max speed
    Pair<Double, Double> maxSpeed = writeXMLSpeed(
        stats.getMaxSpeed(), builder, R.string.description_max_speed_XML, lineBreak);

    // Average pace
    writeXMLPace(averageSpeed, builder, R.string.description_average_pace_XML, lineBreak);

    // Average moving pace
    writeXMLPace(averageMovingSpeed, builder, R.string.description_average_moving_pace_XML, lineBreak);

    // Fastest pace
    writeXMLPace(maxSpeed, builder, R.string.description_fastest_pace_XML, lineBreak);

    // Max elevation
    writeXMLElevation(stats.getMaxElevation(), builder, R.string.description_max_elevation_XML, lineBreak);

    // Min elevation
    writeXMLElevation(stats.getMinElevation(), builder, R.string.description_min_elevation_XML, lineBreak);

    // Elevation gain
    writeXMLElevation(
        stats.getTotalElevationGain(), builder, R.string.description_elevation_gain_XML, lineBreak);

    // Max grade
    writeXMLGrade(stats.getMaxGrade(), builder, R.string.description_max_grade_XML, lineBreak);

    // Min grade
    writeXMLGrade(stats.getMinGrade(), builder, R.string.description_min_grade_XML, lineBreak);

    // Recorded time
    builder.append(
        context.getString(R.string.description_recorded_time_XML,
            StringUtils.formatDateTime(context, stats.getStartTime())));
    builder.append(lineBreak);
...
// Versions for XML writing
   ...
   */
  @VisibleForTesting
  void writeXMLDistance(double distance, StringBuilder builder, int resId, String lineBreak) {
    double distanceInKm = distance * UnitConversions.M_TO_KM;
    double distanceInMi = distanceInKm * UnitConversions.KM_TO_MI;
    builder.append(context.getString(resId, distanceInKm, distanceInMi).replace(",","."));
    builder.append(lineBreak);
  }
 ...
 */
@VisibleForTesting
void writeXMLTime(long time, StringBuilder builder, int resId, String lineBreak) {
  builder.append(context.getString(resId, StringUtils.formatElapsedTime(time)));
  builder.append(lineBreak);
}
 ...
 */
@VisibleForTesting
Pair<Double, Double> writeXMLSpeed(
    double speed, StringBuilder builder, int resId, String lineBreak) {
  double speedInKmHr = speed * UnitConversions.MS_TO_KMH;
  double speedInMiHr = speedInKmHr * UnitConversions.KM_TO_MI;
  builder.append(context.getString(resId, speedInKmHr, speedInMiHr).replace(",","."));
  builder.append(lineBreak);
  return Pair.create(speedInKmHr, speedInMiHr);
}
...
void writeXMLPace(
  ...
  double paceInMinKm = getPace(speed.first);
  double paceInMinMi = getPace(speed.second);
  builder.append(context.getString(resId, paceInMinKm, paceInMinMi).replace(",","."));
  builder.append(lineBreak);
}
 ...
 */
@VisibleForTesting
void writeXMLElevation(
    double elevation, StringBuilder builder, int resId, String lineBreak) {
  long elevationInM = Math.round(elevation);
  long elevationInFt = Math.round(elevation * UnitConversions.M_TO_FT);
  builder.append(context.getString(resId, elevationInM, elevationInFt).replace(",","."));
  builder.append(lineBreak);
}
 ...
 * @param lineBreak line break string
 */
void writeXMLGrade(double grade, StringBuilder builder, int resId, String lineBreak) {
  long gradeInPercent = Double.isNaN(grade) || Double.isInfinite(grade) ? 0L
      : Math.round(grade * 100);
  builder.append(context.getString(resId, gradeInPercent).replace(",","."));
  builder.append(lineBreak);
}
}