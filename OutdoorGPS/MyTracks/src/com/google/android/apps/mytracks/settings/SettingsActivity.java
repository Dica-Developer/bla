...
public class SettingsActivity extends AbstractSettingsActivity {
  ...
  private final OnSharedPreferenceChangeListener sharedPreferenceChangeListener = new OnSharedPreferenceChangeListener() {
    ...
    public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
      ...
      if (PreferencesUtils.getKey(SettingsActivity.this, R.string.user_name).equals(key)) {
        SharedPreferences.Editor editor = preferences.edit();
        String value = PreferencesUtils.getString(SettingsActivity.this, R.string.user_name, "");
  ...
  protected void onCreate(Bundle bundle) {
    ...
    registerPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
      ...
      public boolean onPreferenceClick(Preference preference) {
        ...
        i.setData(Uri.parse(Constants.NOGAGO_REGISTER_URL));
        startActivity(i);
        return true;
      }
    });
    ...
    recordingPreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
      ...
      public boolean onPreferenceClick(Preference preference) {
        Intent intent = IntentUtils.newIntent(SettingsActivity.this,
            RecordingSettingsActivity.class);
        startActivity(intent);
        return true;
      }
    });
    ...
    /*
     * googleAccountListPreference = (ListPreference)
     * findPreference(getString(R.string.google_account_key)); List<String>
     * entries = new ArrayList<String>(); Account[] accounts =
     * AccountManager.get(this).getAccountsByType(Constants.ACCOUNT_TYPE); for
     * (Account account : accounts) { entries.add(account.name); }
     * googleAccountListPreference.setEntries(entries.toArray(new
     * CharSequence[entries.size()]));
     * googleAccountListPreference.setEntryValues(entries.toArray( new
     * CharSequence[entries.size()])); if (entries.size() == 1) {
     * googleAccountListPreference.setValueIndex(0); } String googleAccount =
     * PreferencesUtils.getString(this, R.string.google_account_key,
     * PreferencesUtils.GOOGLE_ACCOUNT_DEFAULT);
     * updateSwitchAccountSummary(googleAccount);
     * googleAccountListPreference.setOnPreferenceChangeListener(new
     * OnPreferenceChangeListener() {
     * @Override public boolean onPreferenceChange(Preference preference, Object
     * newValue) { updateSwitchAccountSummary((String) newValue); return true; }
     * });
     */
  ...
  private void updateSwitchAccountSummary(String value) {
    googleAccountListPreference
        .setSummary(PreferencesUtils.GOOGLE_ACCOUNT_DEFAULT.equals(value) ? getString(R.string.value_unknown)
            : value);
  }
  ...
  protected Dialog onCreateDialog(int id) {
    ...
    return DialogUtils.createConfirmationDialog(this, R.string.settings_reset_confirm_message,
      ...
      public void run() {
        ...
        runOnUiThread(new Runnable() {
          ...
          public void run() {
            ...
            Intent intent = getIntent().addFlags(
