...
public class TrackWriterFactory {
  ...
  public static TrackWriter newWriter(Context context,
      ...
      long trackId, TrackFileFormat format, OutputStream outputStream) {
    Track track = providerUtils.getTrack(trackId);
    if (track == null) {
      return null;
    }

    return newWriter(context, providerUtils, track, format, outputStream);
  }
  ...
  private static TrackWriter newWriter(Context context,
      MyTracksProviderUtils providerUtils,
      Track track, TrackFileFormat format, OutputStream outputStream) {
    TrackFormatWriter writer = format.newFormatWriter(context);
    return new TrackWriterImpl(context, providerUtils, track, writer, outputStream);
  }
