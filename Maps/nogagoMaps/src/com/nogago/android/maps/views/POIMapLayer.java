...
import com.nogago.android.maps.LatLonUtils;
...
import com.nogago.android.maps.activities.EditingPOIActivity;
...
public class POIMapLayer extends OsmandMapLayer implements ContextMenuLayer.IContextMenuProvider {
	...
	private static boolean isOnlineSearching = false;
	private static List<Amenity> amnityList = new ArrayList<Amenity>();
	...
	@Override
	public void onDraw(final Canvas canvas, final RectF latLonBounds, final RectF tilesRect, DrawSettings nightMode) {
		if (view.getZoom() >= startZoom) {
			objects.clear();
			if(view.isCurrentMapOffline()){
				amnityList.clear();
				...
				if(!isOnlineSearching){
					...
					new Thread(new Runnable(){
						@Override
						public void run() {
							List<Amenity> tmpList = new ArrayList<Amenity>();
							RectF newRect = LatLonUtils.getSearchBox(latLonBounds);
							resourceManager.searchAmenitiesOnline(newRect.top, newRect.left, newRect.bottom, newRect.right, filter, tmpList);
							if(!tmpList.isEmpty()){
								amnityList.clear();
								amnityList.addAll(tmpList);
								view.refreshMap();
							}
							isOnlineSearching = false;
						}
					}, "POIOnline SearchThread").start(); //$NON-NLS-1$
				}
				drowPOI(canvas, amnityList);
			}
		}
	}
	
	private void drowPOI(Canvas canvas, List<Amenity> amenities){
		int r = getRadiusPoi(view.getZoom());
		List<Amenity> tmpList = new ArrayList<Amenity>();
		tmpList.addAll(amenities);
		if(objects.isEmpty()) objects.addAll(tmpList);
		
		for (Amenity o : tmpList) {
