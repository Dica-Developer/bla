...
import com.nogago.android.maps.views.MultiTouchSupport.MultiTouchZoomListener;
...
public class OsmandMapTileView extends SurfaceView implements IMapDownloaderCallback, Callback {
	...
	private boolean isCurrentMapOffline;

	public boolean isCurrentMapOffline() {
		return isCurrentMapOffline;
	}

	public void setCurrentMapOffline(boolean isCurrentMapOffline) {
		this.isCurrentMapOffline = isCurrentMapOffline;
	}
...
//					log.info("Draw with layers " + (System.currentTimeMillis() - time));
	...
	public MapControlsLayer getMapControlsLayer(){
		for(OsmandMapLayer layer: layers){
			if(layer instanceof MapControlsLayer) return (MapControlsLayer)layer;
		}
		return null;
	}
	...
	public RouteInfoLayer getRouteInfoLayer(){
		for(OsmandMapLayer layer: layers){
			if(layer != null && layer instanceof RouteInfoLayer) return (RouteInfoLayer)layer;
		}
		return null;
	}
