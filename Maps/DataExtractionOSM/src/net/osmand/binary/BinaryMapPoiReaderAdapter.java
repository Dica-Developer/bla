	...
	private void readPoiData(CollatorStringMatcher matcher, SearchRequest<Amenity> req, PoiRegion region) throws IOException {
		...
		while(true){
			...
			case OsmandOdb.OsmAndPoiBoxData.POIDATA_FIELD_NUMBER:
				...
				if(req.distance<0) withinBound = true;
				else{
					double longitudeX = MapUtils.get31LongitudeX(req.x);
					double latitudeY = MapUtils.get31LatitudeY(req.y);
					withinBound = MapUtils.getDistance(am.getLocation(), latitudeY, longitudeX) <= req.distance;
				}
				if (am != null && withinBound) {
	...
	private Amenity readPoiPoint(int left31, int right31, int top31, int bottom31, 
		...
		while(true){
			...
			case OsmandOdb.OsmAndPoiBoxDataAtom.CATEGORIES_FIELD_NUMBER :
				...
				AmenityType type = AmenityType.AMENITY;
				String subtype = "";
